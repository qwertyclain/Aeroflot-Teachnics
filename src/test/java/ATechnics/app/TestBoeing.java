...
public class TestBoeing {
    ...
    @Autowired
    private BoeingRepository repository;

    @BeforeEach
    public void addTestTasks() {
        BoeingTask task = new BoeingTask();
    ...
    public void addNewTaskCard() throws Exception {
        mockMvc.perform(post("/airbus/add_task")
                        .content(mapper.writeValueAsString(getBoeingTask()))
    ...
    public void editTaskCard() throws Exception {
        mockMvc.perform(patch("/boeing/edit_task/{id}", "1111-11-1")
                        .content(mapper.writeValueAsString(getBoeingTask()))
                        .contentType(MediaType.APPLICATION_JSON))
    ...
    public void getExceptionTaskNotFound() throws Exception {
        mockMvc.perform(patch("/boeing/edit_task/{id}", "1"))
                .andExpect(result ->
    ...
    public void getExceptionFileNotFound() throws Exception {
        mockMvc.perform(post("/boeing"))
                .andExpect(result ->
                        result.getResolvedException().getClass().equals(NotFoundException.class));
    }

    private BoeingTask getBoeingTask() {
        BoeingTask task = new BoeingTask();
